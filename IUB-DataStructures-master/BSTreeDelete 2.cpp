...
public:
    ...
    TreeNode * recDelete(TreeNode *root, float sVal, int &flag)
    {
        TreeNode *delNode, *largest, *croot;
        if(root==NULL) return NULL;
        if(sVal<root->data)
        {
            root->left=recDelete(root->left,sVal,flag);
        }
        else if(sVal>root->data)
        {
            root->right=recDelete(root->right,sVal,flag);
        }
        else{
            if(root->left==NULL)
               ...
               croot->data=largest->data;
               croot->left=recDelete(croot->left,largest->data,flag);
                }
            flag=1;
            return croot;
          }
